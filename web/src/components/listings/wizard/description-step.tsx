...
export function DescriptionStep() {
  ...
  const handleCreateSubcategory = async () => {
    if (!user) {
      alert('Please log in to create a new subcategory');
      return;
    }
    if (!newSubcategoryName.trim() || !selectedMarketplaceId || !selectedCategoryId) return;
            ...
            <div>
              ...
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  disabled={!selectedCategoryId || !user}
                />
                <button
                  type="button"
                  onClick={handleCreateSubcategory}
                  disabled={!selectedCategoryId || !newSubcategoryName.trim() || isCreatingSubcategory || !user}
                  className="px-3 py-2 rounded-md border border-[var(--border-color)] text-xs disabled:opacity-50"
                >
                  {isCreatingSubcategory ? 'Saving...' : 'Add'}
                </button>
              </div>
            </div>
          </div>
