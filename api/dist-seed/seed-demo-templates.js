...
const driveOptionsByMarketplace = {
    agriculture: [
        { value: '4x4', label: '4x4' },
        { value: '2x4', label: '2x4' },
...
function categoryDerivedOptions(categoryName) {
...
async function main() {
    ...
    try {
        ...
        for (const [index, category] of leafCategories.entries()) {
            ...
            console.log(`Template created: ${category.marketplace.name} / ${category.parent?.name ?? '-'} / ${category.name} (template ${template.id.toString()})`);
        }
        console.log(`Done. Created ${createdCount} templates with ${fieldCount} total fields across all leaf subcategories.`);
    }
    finally {
