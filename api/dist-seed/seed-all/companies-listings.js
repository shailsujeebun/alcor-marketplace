...
exports.seedCompaniesAndListings = seedCompaniesAndListings;
const client_1 = require("@prisma/client");
const shared_1 = require("./shared");
async function seedCompaniesAndListings(prisma, input) {
    ...
    async function createListing(params) {
        ...
        const listing = await prisma.listing.create({
            data: {
                ...
                brandId,
                publishedAt: params.publishedOffsetDays !== undefined
                    ? (0, shared_1.daysAgo)(params.publishedOffsetDays)
                    : null,
                attribute: {
                    create: {
                        data: {
                ...
                fact: {
                    create: {
                        priceAmount: new client_1.Prisma.Decimal(params.price),
                        priceCurrency: 'EUR',
                        vatType: 'with_vat',
                        year: params.year,
                        ...
                        lat: new client_1.Prisma.Decimal('50.4501'),
                        lng: new client_1.Prisma.Decimal('30.5234'),
                    },
                },
                media: {
                    create: [
        ...
        await prisma.listingWizardState.create({
            data: {
                ...
                lastSeenAt: (0, shared_1.daysAgo)(1),
            },
        });
        return listing.id;
    }
...
function requireBrandId(map, key) {
    const id = map.get(key);
    if (!id)
        throw new Error(`Missing brand id for ${key}`);
    return id;
}
