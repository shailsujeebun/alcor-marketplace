...
function makeUniqueSlug(base, used, parentSlug) {
    let slug = base;
    if (!used.has(slug))
        return slug;
    if (parentSlug) {
...
async function main() {
    ...
    const cityId = (country, city) => cities[`${country}:${city}`]?.id ?? null;
    ...
    const parentCats = [
        ...
        { slug: 'excavators', name: 'Екскаватори', mp: 'construction' },
    ...
    async function insertTree(nodes, marketplaceKey) {
